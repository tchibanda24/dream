...
module H2_lwt_unix = struct
  ...
  module Server = struct
    ...
    module TLS = struct
      ...
      let create_connection_handler_with_default ~certfile ~keyfile ?config
          ~request_handler ~error_handler =
        let make_tls_server =
          Gluten_lwt_unix.Server.TLS.create_default ~alpn_protocols:["h2"]
            ~certfile ~keyfile
        in
        fun client_addr socket ->
          make_tls_server client_addr socket >>= fun tls_server ->
          create_connection_handler ?config ~request_handler ~error_handler
            client_addr tls_server
    end
    ...
    module SSL = struct
      ...
      let create_connection_handler_with_default ~certfile ~keyfile ?config
          ~request_handler ~error_handler =
        let make_ssl_server =
          Gluten_lwt_unix.Server.SSL.create_default ~alpn_protocols:["h2"]
            ~certfile ~keyfile
        in
        fun client_addr socket ->
          make_ssl_server client_addr socket >>= fun ssl_server ->
          create_connection_handler ?config ~request_handler ~error_handler
            client_addr ssl_server
    end
  end
  ...
  module Client = struct
    ...
    module TLS = struct
      ...
      let create_connection_with_default ?config ?push_handler ~error_handler
          socket =
...
let wrap_handler tls (user's_error_handler : Catch.error_handler)
  ...
  let httpaf_request_handler =
   fun unix_socket client_address (conn : _ Gluten.Reqd.t) ->
    ...
    let httpaf_request : Httpun.Request.t = Httpun.Reqd.request conn in
    ...
    let headers = Httpun.Headers.to_list httpaf_request.headers in

    let body = Httpun.Reqd.request_body conn in
    ...
    in
    let close _code = Httpun.Body.Reader.close body in
           ...
           2. Upon failure to establish a WebSocket, the function is called to
              transmit the resulting error response. *)
                let forward_response response =
                  ...
                  let headers =
                    Httpun.Headers.of_list (Message.all_headers response)
                  in

                  let status = to_httpun_status (Message.status response) in

                  let httpun_response =
                    Httpun.Response.create ~headers status
                  in
                  let body =
                    Httpun.Reqd.respond_with_streaming conn httpun_response
                  in
                ...
                | Some (client_stream, _server_stream) -> (
                  let error_handler =
                    Error_handler.websocket user's_error_handler request
                      response
                  in
                  ...
                  Httpun_ws.Handshake.respond_with_upgrade ~headers ~sha1 conn
                    proceed
                  |> function
                  | Ok () -> Lwt.return_unit
                  | Error error_string ->
                    let%lwt response =
                      Error_handler.websocket_handshake user's_error_handler
                        request response error_string
                    in
            ...
            begin
              fun () ->
                ...
                let forward_response response =
                  ...
                  let headers =
                    H2.Headers.of_list (Message.all_headers response)
                  in
      ...
      begin
        fun ~certificate_file:_ ~key_file:_ ~handler ~error_handler ->
          ...
          fun client_address unix_socket ->
            Httpun_lwt_unix.Server.create_connection_handler ?config:None
              ~request_handler:(request_handler unix_socket)
              ~error_handler client_address unix_socket
      end;
  }
        ...
        fun ~certificate_file ~key_file ~handler ~error_handler ->
          let httpaf_handler =
            ...
            fun client_address unix_socket tls_endpoint ->
              Httpun_lwt_unix.Server.SSL.create_connection_handler ?config:None
                ~request_handler:(request_handler unix_socket)
                ~error_handler client_address tls_endpoint
          in
         ...
         Gluten. Right now, we've picked up a hard dep on OpenSSL. *)
            ...
            | Some tls_socket -> (
             ...
             WebSocket handlers. *)
              ...
              | Some "http/1.1" ->
                httpaf_handler client_address unix_socket tls_endpoint
              | Some "h2" -> h2_handler client_address unix_socket tls_endpoint
              | Some _ -> assert false)
      end;
  }
...
let built_in_middleware error_handler =
  Message.pipeline
    [check_headers_middleware; Catch.catch (Error_handler.app error_handler)]

let serve_with_details caller_function_for_error_messages tls_library ~interface
    ~network ~stop ~error_handler ~certificate_file ~key_file ~builtins
  ...
  let%lwt listen_address =
    match network with
    | `Unix path -> Lwt.return (Lwt_unix.ADDR_UNIX path)
    | `Inet port -> (
      let%lwt addresses =
...
let serve_with_maybe_https caller_function_for_error_messages ~interface
    ~network ~stop ~error_handler ~tls ?certificate_file ?key_file
    ?certificate_string ?key_string ~builtins user's_dream_handler =
  try%lwt
    ...
    | `No ->
      serve_with_details caller_function_for_error_messages no_tls ~interface
        ~network ~stop ~error_handler ~certificate_file:"" ~key_file:""
    ...
    | (`OpenSSL | `OCaml_TLS) as tls_library -> (
      ...
      | `File (certificate_file, key_file) ->
        serve_with_details caller_function_for_error_messages tls_library
          ~interface ~network ~stop ~error_handler ~certificate_file ~key_file
          ~builtins user's_dream_handler
                ...
                begin
                  fun (key_file, key_stream) ->
                    ...
                    serve_with_details caller_function_for_error_messages
                      tls_library ~interface ~network ~stop ~error_handler
...
let run ?(interface = default_interface) ?(port = default_port) ?socket_path
    ?(stop = never) ?(error_handler = Error_handler.default) ?(tls = false)
    ?certificate_file ?key_file ?(builtins = true) ?(greeting = true)
    ?adjust_terminal user's_dream_handler =
  let () =
    if Sys.unix then
  ...
  if greeting then begin
    ...
    begin
      match (interface, socket_path) with
      | _, Some path -> log "Running on %s" path
  ...
  Lwt_main.run
    begin
      serve_with_maybe_https "run" ~interface
        ~network:(network ~port ~socket_path)
        ~stop ~error_handler
        ~tls:(if tls then `OpenSSL else `No)
        ?certificate_file ?key_file ?certificate_string:None ?key_string:None
